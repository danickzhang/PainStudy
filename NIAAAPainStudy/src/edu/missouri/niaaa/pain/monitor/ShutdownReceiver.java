...
public class ShutdownReceiver extends BroadcastReceiver {
	...
	private static final String TAG = "Shutdown Receiver";
	...
	public void onReceive(Context context, Intent intent) {
		...
		String toWrite = MonitorUtilities.getCurrentTimeStamp() + MonitorUtilities.LINEBREAK + "Device is TURNING OFF! And was activated by user!"
		...
		try {
			...
			Log.d(TAG, "monitorEncryption success");
		} catch (Exception e) {
			e.printStackTrace();
	...
	private class TransmitData extends AsyncTask<String, Void, Boolean> {
		...
		protected Boolean doInBackground(String... strings) {
			...
			try {
				...
				Log.d(TAG, "send to server code: "+String.valueOf(response.getStatusLine().getStatusCode()));
